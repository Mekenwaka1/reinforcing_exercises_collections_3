...
for article in collections['response']['results']:
    article['views'] = 0

def read_article():
    random_article = random.choice(collections['response']['results'])

    for idx, article in enumerate(collections['response']['results']):
        if (article['id'] == random_article['id']):
            collections['response']['results'][idx]['views'] += 1
...
def display_views():
    for article in collections['response']['results']:
        print(article["webTitle"], "read", article['views'], "times\n")
